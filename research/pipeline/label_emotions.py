"""E13 主题打标：分层抽样 -> v4-flash 标 9 类情绪编号，断点续跑"""
...
LEAVES = [e["name"] for e in EMO["emotions"]]
...
try:
    done = {json.loads(l)["b"] for l in open(SCRATCH + "/emotions.jsonl")}
...
SYS = ("你是B站视频情绪分类器。判断每个视频标题传递给观众的主导情绪。情绪表（编号=名称）：\n" + menu +
      ...
      for (let att=0; att<3; att++) {
        try {
          ...
          for (const [k,v] of Object.entries(out.r||{})) {
            ...
            if (idx>=0 && idx<chunk.length && tid>=1 && tid<=9) L.out.push({b: chunk[idx].b, tid});
          }
          break;
...
f = open(SCRATCH + "/emotions.jsonl", "a")
t0 = time.time()
while True:
    time.sleep(12)
    try:
